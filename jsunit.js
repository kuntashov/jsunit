...
//Snegopat. 
// Ïî íåèçâåñòíûì ìíå ïðè÷èíàì ýòà êîíñòðóêöèÿ ïðèâîäèò ê òîìó, ÷òî
// äâèæîê Ñíåãîïàòà ïåðåñòàåò âèäåòü ìàêðîñû.
...
JsUnit._setOnLoad = function(windowRef, onloadHandler) {
    ...
    //Snegopat.
    JsUnit.fail("_setOnLoad() íå ïîääåðæèâàåòñÿ â Ñíåãîïàòå.");
    /////Snegopat.
...
JsUnit.Util.standardizeHTML = function(html) {
    //Snegopat.
    JsUnit.fail("standardizeHTML() íå ïîääåðæèâàåòñÿ â Ñíåãîïàòå.");
...
Class.extend = function(def) {
    ...
    for (var n in def) 
    {
        ...
        {
            /* Àëãîðèòì, ðåàëèçóþùèé âîçìîæíîñòü èç ïåðåîïðåäåëåííîãî â ïîòîìêå
             * ìåòîäà ê ìåòîäó áàçîâîãî êëàññà ïðîñòî âûçûâàÿ this._super(...).
             *
             * Êàê ýòî ðàáîòàåò. Ðåãóëÿðíîå âûðàæåíèå â óñëîâèè âûøå - ïðîâåðêà,
             * èñïîëüçóåòñÿ ëè â èñõîäíîì êîäå îïðåäåëåíèÿ ìåòîäà âûçîâ this._super()
             * (ýòî ñâîåãî ðîäà îïòèìèçàöèÿ, ÷òîáû íå äåëàòü îáåðòêè äëÿ âñåõ 
             * ïåðåîïðåäåëåííûõ ìåòîäîâ, à òîëüêî äëÿ òåõ, êîòîðûå èñïîëüçóþò 
             * âûçîâ _super().
             *
             * Åñëè this._super() èñïîëüçóåòñÿ, òî "îáîðà÷èâàåì" èñõîäíûé ìåòîä â  
             * ñïåöèàëüíóþ àíîíèìíóþ ôóíêöèþ, ñóòü ðàáîòû êîòîðîé â ñëåäóþùåì:
             *
             *  1. âðåìåííî ñêîïèðîâàòü â ñâîéñòâî this._super îáúåêòà  ôóíêöèþ, 
             *  ïðîòîòèïà ïðåäâàðèòåëüíî çàïîìíèâ ïðåäûäóùåå çíà÷åíèå ñâîéñòâà;
             *
             *  2. âûçûâàòü îðèãèíàëüíóþ ôóíêöèþ (òó, êîòîðóþ ìû îáåðíóëè); áëàãîäàðÿ 
             *  òîìó, ÷òî â ï. 1, ò.å. ê ìîìåíòó âûçîâà ìû ñîõðàíèëè â this._super 
             *  îäíîèìåííóþ ôóíêöèþ ïðîòîòèïà, òî âíóòðè âûçîâà îðèãèíàëüíîé ôóíêöèè  
             *  îáðàùåíèå ê this._super() áóäåò îçíà÷àòü âûçîâ ìåòîäà ïðîòîòèïà (è  
             *  èìåííî òîãî, êîòîðûé è ïåðåîïðåäåëÿåò îðèãèíàëüíûé ìåòîä).
             * 
             *  3. âîçâðàùàåì èñõîäíîå çíà÷åíèå ñâîéñòâà this._super
             * 
             * "Îáåðòêà" ñîçäàåòñÿ ïðè ïîìîùè äðóãîé àíîíèìíîé ôóíêöèè ñ ïàðàìåòðàìè  
             * name è fn, âûçûâàåìîé ñðàçó â ìåñòå ñîçäàíèÿ - ïðîñòî, ÷òîáû ñîçäàòü, 
             * çàìûêàíèå â êîòîðîì áóäóò çàõâà÷åíû çíà÷åíèÿ ëîêàëüíûõ ïåðåìåííûõ 
             * _super (â íåé - ïðîòîòèï), name - èìÿ îáîðà÷èâàåìîãî ìåòîäà è fn - 
             * ñîáñòâåííî îáîðà÷èâàåìûé ìåòîä.
             */
...
// Âûçîâ áàçîâîãî ìåòîäà:
// this._super(<Ïàðàìåòðû áàçîâîãî ìåòîäà>)
...
if (!Args.Count || !Args.Exists("D")) {
    WScript.Echo("�������������: \n" + 
        "\t cscript //nologo jsunit.js /D:<���� � �������� � �������>");
    WScript.Quit(0);
}

WScript.Echo("!!!:" + Args.Item('D'));
JsUnit.Runner.run(Args.Item('D'));
